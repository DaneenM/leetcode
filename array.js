...
//find in slack
function findMixBreeds () {
    let arrayOfMixBreeds = [];
    for (let i=0; i < arrayOfDogs; i++) {
        if(arrayOfDogs[i].breed.length > 1) {
            arrayOfMixBreeds.push(arrayOfDogs[i])
        };
    };
    return arrayOfMixBreeds
}

console.log(findMixBreeds);